...
bool CRenderToCube::createColorBuffer() {
  ...
  for (int i = 0; i < nsides; ++i) {
    ...
    setDXName(render_target_views[i], side_name);
		...
		//SRVDesc.Texture2DArray.ArraySize = nsides;
		//SRVDesc.Texture2DArray.FirstArraySlice = i;
		//hr = Render.device->CreateShaderResourceView(texture, &SRVDesc, &shader_resource_views[i]);
		//if (FAILED(hr))
		//  return false;
		//sprintf(side_name, "%s.%d.srv", getName().c_str(), i);
		//setDXName(shader_resource_views[i], side_name);
  }
