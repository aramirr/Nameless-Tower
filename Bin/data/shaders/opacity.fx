...
) : SV_Target0
{
  ...
  //  float2 pos_camera_unit_space = float2((1 + pos_homo_space.x) * 0.5, (1 - pos_homo_space.y) * 0.5);
    float4 color_base = txAlbedo.Sample(samClampLinear, iTex0) /** (1. - txTransparency.Sample(samLinear, iTex0))*/; //obj_color.a;
  ...
  // Change the curve of the influence,.
    ...
    final_color *= txTransparency.Sample(samLinear, iTex0); //obj_color.a;
    return final_color;
