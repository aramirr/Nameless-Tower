...
)
{
  ...
  //world_pos.xyz += oNormal + ( 5 * global_world_time + world_pos.y * 0.2);

  oPos = mul(world_pos, camera_view_proj);

    //float width;
    //float height;
    //txAlbedo.GetDimensions(width, height);
...
)  : SV_Target0
{
 ...
 // float2 pos_camera_unit_space = float2( ( 1 + pos_homo_space.x ) * 0.5, ( 1 - pos_homo_space.y ) * 0.5 );
    float4 color_base = txAlbedo.Sample(samLinear, iTex0);// * obj_color.a;
