...
//--------------------------------------------------------------------------------------
// Opacity generation pass. Vertex
//--------------------------------------------------------------------------------------
void VS_Opacity(
  in float4 iPos : POSITION
	, in float3 iNormal : NORMAL0
...
) : SV_Target0
{
  ...
  // Convert world coords to camera space in the range 0..1 to access the rt_main texture
    ...
    float4 color_base = txAlbedo.Sample(samClampLinear, iTex0) * txTransparency.Sample(samClampLinear, iTex0); //obj_color.a;
  ...
  // Change the curve of the influence,.
    ...
    float roughness = 0.5f;//length(txRoughness.Sample(samLinear, iTex0));
    ...
    final_color *= txTransparency.Sample(samClampLinear, iTex0); //obj_color.a;
    return final_color;
